...
function AddRowInCart(productTitle, productPrice, productimg) {
    ...
    insideDiv = `<div class="productImage"><img src="document.querySelector{productimg}"></div>
    <div class="productTitle"> document.querySelector{productTitle} </div>
    <div class="productPrice"> document.querySelector{productPrice}</div>
 <div class="col-xs-3 removeButton"> Remove</div>`;
    ...
    for (var i = 0; i < cartPrice.length; i++) {
        priceFormatedNumb = cartPrice[i].innerText.replace('R', '').replace(',', '');
